...
export default {
  ...
  computed: {
    url() {
      ...
      return `${prefix + (this.room || this.defaultName)}?audio=${Number(
        this.presetAudio,
      )}&video=${Number(this.presetVideo)}&fs=${Number(
        this.presetFullscreen,
      )}&invite=${Number(this.presetInvite)}&prefs=${Number(
        this.presetPrefs,
      )}&share=${Number(this.presetShare)}&chat=${Number(this.presetChat)}`;
    },
    code() {
      return `<iframe 
  src="${this.url}"
