...
async function inBrowserTest() {
  ...
  log.assert((await digestMessage(fp)) === 'MT2AATY56');
  log.assert(splitByNChars('MT2AATY56') === 'MT2-AAT-Y56');

  const r1 = await digestMessages('abc', fp);
  const r2 = await digestMessages(fp, 'abc');
  log.assert(r1 === r2);
}
