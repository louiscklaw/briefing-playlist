...
export class Peer extends Emitter<{
  ...
  setStream(stream: MediaStream) {
    ...
    for (const activeStream of this.streams) {
      if (activeStream.id !== stream.id) this.removeStream(activeStream);
    }

    if (!this.hasStream(stream)) this.addStream(stream);
  }
  ...
  _onIceStateChange() {
    ...
    if (iceConnectionState === 'disconnected') this.emit('disconnect');
    ...
    event.streams.forEach((eventStream: MediaStream) => {
      ...
      if (
        this._remoteStreams.some(
          remoteStream => remoteStream.id === eventStream.id,
        )
      )
        return;
