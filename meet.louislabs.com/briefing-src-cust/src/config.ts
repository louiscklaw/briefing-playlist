// https://github.com/holtwick/briefing/blob/master/docs/configuration.md
...
const normalizeConfigName = (name: string) =>
  name.toUpperCase().replace(/[-\ .]/gim, '_');
...
export function getConfig(
  ...
  if (forceUpdate || configNormalized == null) {
    const configEnv = Object.entries(import.meta.env)
      .map(([key, value]) => {
        key = normalizeConfigName(key);
        if (key.startsWith('BRIEFING_')) return [key, value];
        return undefined;
      })
      .filter(isValue);

    const configWindow = Object.entries(window.briefingConfig ?? {}).map(
      ([key, value]) => [`BRIEFING_${normalizeConfigName(key)}`, value],
    );

    configNormalized = Object.fromEntries([...configEnv, ...configWindow]);
  }

  return (
    configNormalized[`BRIEFING_${normalizeConfigName(name)}`] ?? defaultValue
  );
}
...
export const MUTE_VIDEO = isTrue(getConfig('MUTE_VIDEO'), true);
