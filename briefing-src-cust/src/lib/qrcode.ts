...
const qrcode = (function () {
  ...
  const qrcode = function (typeNumber, errorCorrectionLevel) {
    ...
    _this.createSvgTag = function (cellSize, margin, alt, title) {
      ...
      qrSvg +=
        title.text || alt.text
          ? ` role="img" aria-labelledby="${escapeXml(
              [title.id, alt.id].join(' ').trim(),
            )}"`
          : '';
      qrSvg += '>';
      qrSvg += title.text
        ...
        : '';
      qrSvg += alt.text
        ? `<description id="${escapeXml(alt.id)}">${escapeXml(
            alt.text,
          )}</description>`
        : '';
  ...
  var qrKanji = function (data) {
    ...
    _this.write = function (buffer) {
      ...
      while (i + 1 < data.length) {
        ...
        if (c >= 0x8140 && c <= 0x9ffc) c -= 0x8140;
        else if (c >= 0xe040 && c <= 0xebbf) c -= 0xc140;
  ...
  var base64DecodeInputStream = function (str) {
    ...
    var decode = function (c) {
      ...
      else if (c >= 0x61 && c <= 0x7a) return c - 0x61 + 26;
      else if (c >= 0x30 && c <= 0x39) return c - 0x30 + 52;
      else if (c == 0x2b) return 62;
      else if (c == 0x2f) return 63;
      else throw `c:${c}`;
    };
  ...
  var createDataURL = function (width, height, getPixel) {
    ...
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) gif.setPixel(x, y, getPixel(x, y));
    }
